...
//对以指针p所指节点为根的二叉树作左平衡旋转处理,本算法结束时,指针p指向新的根节点
{
	BSTNode *p1,*p2;
	...
	else					//原本左子树比右子树高,需作左子树的平衡处理
	{
		...
		if (p1->bf==1)		//新节点插入在*b的左孩子的左子树上,要作LL调整
		{
			p->lchild=p1->rchild;
			p1->rchild=p;
			p->bf=p1->bf=0;
		...
		else if (p1->bf==-1)	//新节点插入在*b的左孩子的右子树上,要作LR调整
		{
			p2=p1->rchild;
			p1->rchild=p2->lchild;
			p2->lchild=p1;
			p->lchild=p2->rchild;
			p2->rchild=p;
			if (p2->bf==0)			//新节点插在*p2处作为叶子节点的情况
				p->bf=p1->bf=0;	
			else if (p2->bf==1)		//新节点插在*p2的左子树上的情况
			{
				p1->bf=0;p->bf=-1;
			}
			else					//新节点插在*p2的右子树上的情况
			{
				p1->bf=1;p->bf=0;
			}
			p=p2;p->bf=0;			//仍将p指向新的根节点,并置其bf值为0
		}
		taller=0;
	}
}
void RightProcess(BSTNode *&p,int &taller)
//对以指针p所指节点为根的二叉树作右平衡旋转处理,本算法结束时,指针p指向新的根节点
{
	BSTNode *p1,*p2;
	...
	{
		p1=p->rchild;		//p指向*p的右子树根节点
		if (p1->bf==-1)		//新节点插入在*b的右孩子的右子树上,要作RR调整
		{
			p->rchild=p1->lchild;
			p1->lchild=p;
			p->bf=p1->bf=0;
		...
		else if (p1->bf==1)	//新节点插入在*p的右孩子的左子树上,要作RL调整
		{
			p2=p1->lchild;
			p1->lchild=p2->rchild;
			p2->rchild=p1;
			...
			if (p2->bf==0)			//新节点插在*p2处作为叶子节点的情况
				p->bf=p1->bf=0;	
			else if (p2->bf==-1)	//新节点插在*p2的右子树上的情况
			{
				p1->bf=0;p->bf=1;
			}
			else					//新节点插在*p2的左子树上的情况
			{
				p1->bf=-1;p->bf=0;
			}
			p=p2;p->bf=0;			//仍将p指向新的根节点,并置其bf值为0
...
/*若在平衡的二叉排序树b中不存在和e有相同关键字的节点,则插入一个
  数据元素为e的新节点,并返回1,否则返回0。若因插入而使二叉排序树
  失去平衡,则作平衡旋转处理,布尔变量taller反映b长高与否*/
{
	  if(b==NULL)			//原为空树,插入新节点,树“长高”,置taller为1
	{ 
    ...
    {
		if (e==b->key)				//树中已存在和e有相同关键字的节点则不再插入
		{ 
			taller=0;
			return 0;
		}
...
void LeftProcess1(BSTNode *&p,int &taller)	//在删除节点时进行左处理
{
	BSTNode *p1,*p2;
	if (p->bf==1)
	{
...
void RightProcess1(BSTNode *&p,int &taller) //在删除节点时进行右处理
{
	BSTNode *p1,*p2;
	if (p->bf==-1)
	{
...
//由DeleteAVL()调用,用于处理被删节点左右子树均不空的情况
{
	if (r->rchild==NULL)
	{
		q->key=r->key;
...
int DeleteAVL(BSTNode *&p,KeyType x,int &taller) //在AVL树p中删除关键字为x的节点
{
	int k;
	BSTNode *q;
	if (p==NULL) 
	...
	else			//找到了关键字为x的节点,由p指向它
	{
		...
		else if (p->lchild==NULL)	//被删节点左子树为空
		{
			p=p->rchild;
			free(q);
			taller=1;
